...
if __name__ == '__main__':
    start = time.time()
    #创建数据集
    datMat = mat(loadDataSet('F:/pytorchwork/Statistical-Learning-Method/data/testSet.txt'))

    print(min(datMat[:, 0]))

    # randCent() 函数能否生成 min 到 max 之间的值
    print(randCent(datMat, 4))
    # 最后测试一下距离计算方法
    a1=distEclud(datMat[0], datMat[1])
    print(a1)

    kMeans(datMat, 5, distMeas=distEclud, createCent=randCent)
    biKmeans(datMat, 5, distMeas=distEclud)


    myCent, clustAssing = kMeans(datMat, 5)
    plotPoint(datMat, myCent, clustAssing)

    #dataMat = mat(loadDataSet('testSet2.txt'))
    myCent, clustAssing = biKmeans(datMat, 3)
    plotPoint(datMat, myCent, clustAssing)
    end = time.time()
